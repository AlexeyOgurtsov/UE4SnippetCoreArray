...
void UMyArrayLib::Array_FindByPredicate()
{
	UE_LOG(MyLog, Log, TEXT("Array_FindByPredicate..."));

	TArray<int32> Array { 3, 7, 1, 6, 2 };
	const int32* pEven = Array.FindByPredicate([](int32 const InValue){ return InValue % 2 == 0; });
	int32 const EvenIndex = Array.IndexOfByPredicate([](int32 const InValue){ return InValue % 2 == 0; });
	if(pEven)
	{
		check(EvenIndex != INDEX_NONE);
		UE_LOG(MyLog, Log, TEXT("Even found: %d, index=%d"), *pEven, EvenIndex);
	}
	else
	{
		UE_LOG(MyLog, Log, TEXT("Even NOT found"));
	}

	UE_LOG(MyLog, Log, TEXT("Array_FindByPredicate DONE"));
}
